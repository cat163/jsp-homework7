...
import com.diary.model.CityMap;
...
public class UserServlet extends HttpServlet {
	...
	private void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (r > 0) { // 当用户登录成功时
			...
			session.setAttribute("uid", f.getId()); // 保存用户ID
	...
	public void checkUser(HttpServletRequest request, HttpServletResponse response)
		...
		String sql = "SELECT * FROM tb_user WHERE username='" + username + "'";			// SQL 语句
	...
	public void getCity(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		result = result.substring(0, result.length() - 1); // 去除最后一个逗号
		response.setContentType("text/html;charset=utf-8");// 向响应的内容类型
		PrintWriter out = response.getWriter();
